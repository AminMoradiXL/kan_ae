...
#             nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1),  # (N, 3, 32, 32) -> (N, 16, 16, 16)
#             nn.ReLU(),
#             nn.Conv2d(16, 8, kernel_size=3, stride=2, padding=1),  # (N, 16, 16, 16) -> (N, 32, 8, 8)
#             nn.ReLU(),
...
#             nn.ReLU(),
#             nn.ConvTranspose2d(8, 16, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 32, 8, 8) -> (N, 16, 16, 16)
#             nn.ReLU(),
#             nn.ConvTranspose2d(16, 3, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 16, 16, 16) -> (N, 3, 32, 32)
#             nn.Tanh()
#         )
...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        # Encoder layers
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1),  # (N, 3, 32, 32) -> (N, 16, 16, 16)
            nn.ReLU(),
            nn.Conv2d(64, 8, kernel_size=3, stride=2, padding=1),  # (N, 16, 16, 16) -> (N, 32, 8, 8)
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(8, 64, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 32, 8, 8) -> (N, 16, 16, 16)
            nn.ReLU(),
            nn.ConvTranspose2d(64, 3, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 16, 16, 16) -> (N, 3, 32, 32)
            nn.Tanh()
        )
...
valset = SVHN(root="./data", split='test', download=True, transform=transform)
