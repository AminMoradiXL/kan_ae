...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 2, kernel_size=3, stride=2, padding=1),  # (N, 1, 28, 28) -> (N, 2, 14, 14)
            nn.ReLU(),
            nn.Conv2d(2, 4, kernel_size=3, stride=2, padding=1),  # (N, 2, 14, 14) -> (N, 4, 7, 7)
            nn.ReLU(),
            nn.Conv2d(4, 4, kernel_size=3, stride=2, padding=1)   # (N, 4, 7, 7) -> (N, 4, 4, 4)
        )
        
        # Decoder layers
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(4, 4, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 4, 4, 4) -> (N, 4, 7, 7)
            nn.ReLU(),
            nn.ConvTranspose2d(4, 2, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 4, 7, 7) -> (N, 2, 14, 14)
            nn.ReLU(),
            nn.ConvTranspose2d(2, 1, kernel_size=3, stride=2, padding=1, output_padding=1),  # (N, 2, 14, 14) -> (N, 1, 28, 28)
            nn.Tanh()
        )
